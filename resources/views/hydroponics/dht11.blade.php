...
@section('title', 'DHT11 Temperature & Humidity Dashboard')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div class="p-6 flex items-center">
                    ...
                    <div>
                        <span class="block text-4xl font-bold">{{ end($sensorData)['temperature'] ?? 'N/A' }}°C</span>
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div class="p-6 flex items-center">
                    ...
                    <div>
                        <span class="block text-4xl font-bold">{{ end($sensorData)['humidity'] ?? 'N/A' }}%</span>
                        <span class="text-gray-500 text-sm">Last updated: {{ end($sensorData)['timestamp'] ?? 'N/A' }}</span>
                    </div>
                </div>
            </div>
...
<span class="h-2 w-2 rounded-full {{ (isset($reading['temperature']) && $reading['temperature'] !== null && $reading['temperature'] > 30) ? 'bg-red-500' : 'bg-green-500' }} mr-2"></span>
                                        {{ $reading['temperature'] ?? 'N/A' }}°C
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <span class="inline-flex items-center">
<span class="h-2 w-2 rounded-full {{ (isset($reading['humidity']) && $reading['humidity'] !== null && $reading['humidity'] > 70) ? 'bg-blue-500' : 'bg-green-500' }} mr-2"></span>
...
<span class="px-2 py-1 text-xs font-semibold rounded-full
                                    {{ ((isset($reading['temperature']) && $reading['temperature'] !== null && $reading['temperature'] > 30) || (isset($reading['humidity']) && $reading['humidity'] !== null && $reading['humidity'] > 70)) ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800' }}">
                                        {{ ((isset($reading['temperature']) && $reading['temperature'] !== null && $reading['temperature'] > 30) || (isset($reading['humidity']) && $reading['humidity'] !== null && $reading['humidity'] > 70)) ? 'Warning' : 'Normal' }}
                                    </span>
                                </td>
                            </tr>
                        @endforeach
    ...
    <!-- Data Visualization -->
    @if($sensorData && count($sensorData) > 1)
